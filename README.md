...
히어로는 **픽셀 아트**입니다. 연속 계조로 그린 뒤 그레인을 얹는 방식으로는 이 재질이 나오지
않습니다. 파이프라인 전체가 픽셀 그리드 위에서 동작합니다.

```
seed ──> L-system 골격 ──┬──> 저해상도 네이티브 렌더 (960×420 등)
                         │      · 안티에일리어싱 없음 (feather 0)
                         │      · 입자 = 1~2px 정사각 도트
                         │      · 16색 램프 + 베이어 오더드 디더
                         │    ──> PNG(팔레트) / WebP(무손실), 약 60KB
                         │    ──> CSS image-rendering: pixelated 로 업스케일
                         └──> tree-skeleton.json (63KB gz) ──> 브라우저에서 포인트 클라우드로 확장
```

핵심 규칙 세 가지:

1. **손실 압축 금지.** AVIF/WebP 손실 인코딩은 셀 경계를 뭉개고 램프 밖의 색을 만들어냅니다.
   이 디자인은 그 경계가 전부입니다.
2. **리사이즈 사다리 없음.** 네이티브 해상도로 그대로 보내고 브라우저가 nearest-neighbor로
   확대합니다. 어떤 뷰포트에서도 픽셀이 정사각으로 유지되고, 파일도 훨씬 작습니다.
3. **WebGL 입자도 같은 그리드.** 포인트는 원형 스프라이트가 아니라 셀에 스냅된 정사각형이며,
   크기는 셀의 정수배입니다. 그렇지 않으면 픽셀 아트 위에 부드러운 헤일로가 얹힙니다.
...
| 히어로 포스터 | 350–500KB | 약 60KB (네이티브 PNG, 16색 팔레트) |
